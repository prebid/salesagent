...
**Round 13/14 amendment:** Solo-maintainer SPOF acceptance is now ON CRITICAL PATH (not just a tripwire). Per A25 pre-flight, before PR 3 Phase B execution, confirm @chrishuie has hardware MFA enabled (FIDO2 / hardware key) and document SPOF acceptance via `runbooks/E4-account-lockout-recovery.md` for the unavailability case. Under the solo+agents execution model, recruiting a second maintainer is not an available path (agents are not maintainers); hardware MFA + documented runbook is the only mitigation. The R20/R30 CRITICAL-severity risks both depend on @chrishuie not being compromised AND not being unavailable; Round 13 comprehensive review elevated this from "tripwire" to "blocker."
...
- **Lockout recovery (Round 14 M7):** if @chrishuie's GitHub account becomes inaccessible (lost MFA, SSO break, GitHub-side outage), see `runbooks/E4-account-lockout-recovery.md` for the documented recovery procedure. ADR-002 acknowledges the SPOF; this runbook covers the unavailability case A25 elevated to in-scope.
...
- **Sustainability tripwire:** if maintainer review backlog reaches 5+ open Dependabot PRs, pause forward work on issue #1234 and clear backlog. If backlog persists across multiple weeks, escalate to pausing forward work until backlog clears; see `runbooks/E1-dependabot-backlog-tripwire.md` for the realistic options (tighter dependabot grouping, reduced churn, threshold-raise via follow-up ADR). Solo+agents execution model means human-team scaling is not a recourse.
...
- **Operational definition of "blocked" (Round 14 B3):** v2.0 is "blocked" if `gh api repos/prebid/salesagent/pulls/1221 --jq '.merged'` returns false AND `gh pr list --label v2.0 --base main --state merged` is empty for ≥14 calendar days from issue #1234 PR 1 review start. If tripwire fires, decision authority is @chrishuie; consult `briefings/path-2-fallback.md` for impact assessment.
...
**Wiring contract (Round 12 R12A-01 fix — load-bearing; per-branch defaults clarified Round 14 B1):** `src/core/database/database_session.py:108-109` (PgBouncer branch — currently `pool_size=2, max_overflow=5`) and `:124-125` (direct PG branch — currently `pool_size=10, max_overflow=20`) hardcode `pool_size` and `max_overflow` as Python literals. Without app-side wiring, the env vars are ignored and D40's mitigation silently no-ops. **PR 2 gains a new commit** (between current 4.5 and 5) that wires the env vars on **BOTH** branches, **preserving each branch's existing default literal** (so production behavior is unchanged when env vars are unset):

```python
# src/core/database/database_session.py — wire env-var override on BOTH branches:

# Lines 108-109 (PgBouncer branch — keep defaults 2/5):
pool_size=int(os.getenv("DB_POOL_SIZE", "2")),
max_overflow=int(os.getenv("DB_MAX_OVERFLOW", "5")),

# Lines 124-125 (direct PG branch — keep defaults 10/20):
pool_size=int(os.getenv("DB_POOL_SIZE", "10")),
max_overflow=int(os.getenv("DB_MAX_OVERFLOW", "20")),
```

**Defaults preserve existing production behavior** (PgBouncer keeps `(2, 5)`; direct PG keeps `(10, 20)`). CI overrides via `_pytest/action.yml` env block. CI uses port 5432 → CI hits the **direct PG branch** specifically, where `DB_POOL_SIZE=4` represents a scale-down from default 10 (not a scale-up from PgBouncer's 2). `verify-pr2.sh` and `verify-pr3.sh` enforce both `DB_POOL_SIZE` AND `DB_MAX_OVERFLOW` are wired, AND that the per-branch defaults are exactly `(2|10)` and `(5|20)` (production regression catch — see Round 14 B1).
...
**Decided 2026-04-26 (Round 11 sweep — R11C-06; rationale corrected Round 14 B4):** PR 4 commit 1 (in addition to `default_install_hook_types: [pre-commit, pre-push]` per D31) adds `minimum_pre_commit_version: 3.2.0` to the top of `.pre-commit-config.yaml`.

- **Rationale:** PR 4 uses the **modern stage names** (`pre-push` in `default_install_hook_types` per D31, AND `stages: [pre-push]` on 10 hooks per Commit 5). The `pre-push` stage name was introduced in **pre-commit 3.2.0** (March 2023, soft-aliased the legacy `push` name); on pre-commit 2.x–3.1.x, `pre-push` is an unrecognized stage value and the affected hooks silently do not register. NOTE (corrected Round 14 B4): `default_install_hook_types` itself has been available since pre-commit 2.11.0 (March 2021); the floor is 3.2.0 for **stage-name recognition**, NOT for the directive itself. On pre-commit <3.2, a contributor running `pre-commit install` would (a) see `[WARNING] Unexpected key(s) present at root: default_install_hook_types` (a warning, not an error — easily missed in install noise; pre-commit emits a warning rather than silently ignoring), AND (b) hit unrecognized `pre-push` stage values, ending up with the entire pre-push tier disabled. The `minimum_pre_commit_version` directive (a top-level key available since pre-commit 1.15.0) raises a `FatalError` at config-load time and exits non-zero, making the version mismatch loud rather than warning-noise.
- **Tripwire:** if a contributor reports that `pre-commit install` fails with a version-too-old `FatalError`, point them at `uv tool install pre-commit` or `pip install --upgrade pre-commit`.
- **Affected:** PR 4 commit 1; PR 4 also updates `docs/development/contributing.md` to mention the 3.2.0 minimum and the upgrade commands (Round 14 B4 — neither `CONTRIBUTING.md` nor `docs/development/contributing.md` currently mentions any pre-commit version floor; PR 4 must add this).
...
- **Tripwire:** if a future Phase B-class operation is unavoidable on a weekend, require a temporarily-added admin via E4 runbook's Option A (sandbox / temporary admin grant with documented revoke time). This must NOT be confused with a permanent co-maintainer recruit (impossible under solo+agents execution model per ADR-010). Document in an explicit ADR before executing.
...
## D48 — Production deploy coupling: Fly.io documented; cosign verify NOT enforced on Fly side

**Decided 2026-04-26 (Round 13 comprehensive review):** The salesagent app is deployed to Fly.io as `adcp-sales-agent`, pulling from `ghcr.io/prebid/salesagent:vX.Y.Z`. PR 6 adds cosign keyless signing on every release. Fly.io does NOT currently `cosign verify` before pulling, so cosign signing failures do NOT block production deploy. D47 gates publish on CI green, so red main does NOT auto-publish. Production rollback is `fly deploy --image ghcr.io/prebid/salesagent:v<previous>` (direct, no CI involvement).

**Why:** Comprehensive review (Round 13) flagged that the planning corpus made zero references to Fly.io despite `scripts/deploy/fly-set-secrets.sh` existing. Without explicit acknowledgment, the production blast radius is invisible to anyone reading the corpus cold — the answer must be in the corpus, not in the executor's head.

**Tripwire:** If Fly.io ever enables `cosign verify` (post-#1234), Sigstore Rekor outages become production-deploy outages. At that point, R40 (cosign+rekor cascade) escalates from informational to critical, and a "Sigstore unavailable" runbook becomes urgent.

**References:** EXECUTIVE-SUMMARY.md §"Production Deploy Coupling"; A24 (dry-run); R40 (cosign+rekor cascade); R44 (signed-but-broken image, closed by D47).
...
- 2026-04-26 (Round 13 comprehensive review) — D48 added (Production Deploy Coupling: Fly.io pulls from ghcr.io tag scheme; cosign verify NOT enforced on Fly side; D47 gates publish on CI green; rollback via fly deploy --image). D1 amended (solo-maintainer SPOF acceptance elevated from "tripwire" to "ON CRITICAL PATH" via A25). R45 added (actions/cache cross-PR poisoning via fork-PR cache scope). R46 added (GHA usage-minute spike during Phase A overlap). R47 added (status-check name propagation lag at Phase B flip). A24 added (Phase B dry-run on sandbox repo — BLOCKER for PR 3 Phase B). A25 added (hardware MFA on bypass actor — BLOCKER for PR 3 Phase B). A26 added (later deleted in Round 14 reframe — solo+agents doesn't need notification routing). EXECUTIVE-SUMMARY.md gains Production Deploy Coupling, Cost of Operations, and Alerting Strategy sections. Parallel-agent execution scaffolding files created: COORDINATION.md (PR-claiming registry), REBASE-PROTOCOL.md (mandatory rebase order for shared files); ONBOARDING-CHEAT-SHEET.md and FAILURE-BROADCAST-PROTOCOL.md were also created but later removed in Round 14 reframe. P9 grep-guard PATTERNS extended.
...
- 2026-04-27 (Round 14 reframe — agent-team execution model) — Per user feedback ("It is me and agents reviewing the work"), stripped 40-person team / CTO / VP-Eng / boss-level / multi-team scaffolding per `feedback_agent_team_execution_model.md`. Deleted `ONBOARDING-CHEAT-SHEET.md` (duplicated orientation) and `FAILURE-BROADCAST-PROTOCOL.md` (assumed human-team Slack/PR-comments). Kept `COORDINATION.md` and `REBASE-PROTOCOL.md` (useful for parallel-agent execution). A25 simplified to hardware-MFA-only path (Option A recruit-second-maintainer was impossible under solo+agents execution model). A26 deleted entirely. Also closed Round 14 findings: B1 (D40 per-branch defaults), B4 (D44 rationale corrected), M1 (25 verify-script patches), M2/M3/M6 (propagation drift sweep + check-stale-strings.sh fixes), M4 (R11-R15 RESERVED), M5 (P10 hook-baseline pre-flight), M7 (E4 lockout runbook), M8 (PR 6 emergency-revert scratch test), B3 (Path 2 fallback briefing + D20 operational definition).
