...
**Round 13 amendment:** Recruit second maintainer is now ON CRITICAL PATH (not just a tripwire). Per A25 pre-flight, before PR 3 Phase B execution, EITHER recruit a second maintainer to the bypass list (preferred) OR confirm @chrishuie has hardware MFA. The R20/R30 CRITICAL-severity risks both depend on @chrishuie not being compromised AND not being unavailable; Round 13 CTO-level review elevated this from "tripwire" to "blocker."
...
**Decided 2026-04-26 (Round 13 boss-level review):** The salesagent app is deployed to Fly.io as `adcp-sales-agent`, pulling from `ghcr.io/prebid/salesagent:vX.Y.Z`. PR 6 adds cosign keyless signing on every release. Fly.io does NOT currently `cosign verify` before pulling, so cosign signing failures do NOT block production deploy. D47 gates publish on CI green, so red main does NOT auto-publish. Production rollback is `fly deploy --image ghcr.io/prebid/salesagent:v<previous>` (direct, no CI involvement).

**Why:** Boss-level review (Round 13 simulating CTO/VP-Eng with 40-person team) flagged that the planning corpus made zero references to Fly.io despite `scripts/deploy/fly-set-secrets.sh` existing. Without explicit acknowledgment, the boss's first 60-second question would be "what's the production blast radius?" — answer must be in the corpus, not in the executor's head.
...
- 2026-04-26 (Round 13 boss-level / multi-team review) — D48 added (Production Deploy Coupling: Fly.io pulls from ghcr.io tag scheme; cosign verify NOT enforced on Fly side; D47 gates publish on CI green; rollback via fly deploy --image). D1 amended (recruit second maintainer elevated from "tripwire" to "ON CRITICAL PATH" via A25). R45 added (actions/cache cross-PR poisoning via fork-PR cache scope). R46 added (GHA usage-minute spike during Phase A overlap). R47 added (status-check name propagation lag at Phase B flip). A24 added (Phase B dry-run on sandbox repo — BLOCKER for PR 3 Phase B). A25 added (recruit second maintainer OR confirm hardware MFA on bypass actor — BLOCKER for PR 3 Phase B). A26 added (configure notification routing for 40-person team). EXECUTIVE-SUMMARY.md gains Production Deploy Coupling, Cost of Operations, and Alerting Strategy sections. Multi-team execution scaffolding files created: COORDINATION.md (PR-claiming registry), REBASE-PROTOCOL.md (mandatory rebase order for shared files), ONBOARDING-CHEAT-SHEET.md (10-minute orientation), FAILURE-BROADCAST-PROTOCOL.md (escalation comms protocol). P9 grep-guard PATTERNS extended.
