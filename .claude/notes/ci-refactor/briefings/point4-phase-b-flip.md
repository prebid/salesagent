...
- `.github/workflows/ci.yml` exists and emits the 14 frozen check names per D17 amended by D30 — `CI / Quality Gate`, `CI / Type Check`, `CI / Schema Contract`, `CI / Unit Tests`, `CI / Integration Tests`, `CI / E2E Tests`, `CI / Admin UI Tests`, `CI / BDD Tests`, `CI / Migration Roundtrip`, `CI / Coverage`, `CI / Summary`, `CI / Smoke Tests`, `CI / Security Audit`, `CI / Quickstart`
- `.github/actions/_pytest/action.yml` composite action exists (NOT a reusable workflow — Decision-4 eliminates the 3-segment rendered-name issue at design time)
...
- `.coverage-baseline` exists with contents `53.5` (D11 — hard-gate from PR 3 day 1; ratchet upward only when measured-stable)
...
4. The exact 14 names you'll PATCH in match what `ci.yml` emits (BARE form per D26 — workflow `name: CI` + job `name: 'Quality Gate'` renders as `CI / Quality Gate`):
   ```bash
...
| 4. Compose the `gh api -X PATCH` body | Agent | **Use `bash scripts/flip-branch-protection.sh` for the actual PATCH operation. Do NOT manually copy the JSON body — the script ensures idempotency, captures snapshot, prompts for FLIP confirmation, and uses the canonical 14-context list per D17 amended by D30.** |
...
| 6. Verify the flip via diff | Agent | `gh api repos/prebid/salesagent/branches/main/protection/required_status_checks --jq '.checks[].context' | sort | diff - <(printf "%s\n" "CI / Admin UI Tests" "CI / BDD Tests" "CI / Coverage" "CI / E2E Tests" "CI / Integration Tests" "CI / Migration Roundtrip" "CI / Quality Gate" "CI / Quickstart" "CI / Schema Contract" "CI / Security Audit" "CI / Smoke Tests" "CI / Summary" "CI / Type Check" "CI / Unit Tests" \| sort)` |
| 7. Open trivial test PR | Either (user typically) | confirm all 14 new names show as required, all green |
...
4. Verify ci.yml emits the 14 names (Step 3 above)
5. **Use `bash .claude/notes/ci-refactor/scripts/flip-branch-protection.sh` for the actual PATCH operation. Do NOT manually copy the JSON body** — the script ensures idempotency, captures snapshot, prompts for FLIP confirmation, and uses the canonical 14-context list per D17 amended by D30.
6. Hand control to user (script's `read -p "FLIP"` prompt requires user execution; rollback command is in script's failure path)
...
- R8: coverage drop after CI restructure — coverage is hard-gated from PR 3 day 1 (per D11); a single PR dropping >2pp blocks merge with a clear failure message. No advisory window.

**Escalation triggers**
- Phase A overlap shows ANY new check failing (not green) on main: STOP, do not flip; investigate flake vs real failure
- The 14 names emitted by `ci.yml` differ from D17 amended by D30 (typo, different casing, missing): STOP, fix in a small PR before flipping
- After flip, trivial test PR shows any of the 14 names as "expected but not running": STOP, run inverse PATCH within 5 minutes
