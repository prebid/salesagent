...
**You CANNOT do.** Phase B's `gh api -X PATCH` flip — admin-only. Phase C's `test.yml` deletion — separate follow-up PR. Modifying branch-protection in any way. Adding required checks beyond the 14 frozen names per D17 — that's a contract.
...
**Escalation triggers.**
- New `ci.yml` doesn't produce all 14 frozen check names on a real PR (Phase A soak fails) → STOP, file escalation.
