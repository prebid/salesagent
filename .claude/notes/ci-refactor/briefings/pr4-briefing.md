...
**Concurrent activity.** v2.0 phase PRs landing on `.pre-commit-config.yaml` are HIGHEST conflict; coordinate. v2.0 contributes **27 architecture tests + 4 top-level + 9 baseline JSONs** (Round 8 drift-verified — was claimed 31/9, corrected to 27/4/9). Per D18 (revised Round 8), post-v2.0-rebase guard count is **~81** (was ~73). PR 4's CLAUDE.md table audit DEFERS to a post-v2.0-rebase commit; PR 4 commit 9 adds only **1 residual row** (`production_session_add` — v2.0 deletes `no_silent_except`, removing it from the residual list).
...
2. CLAUDE.md table on disk (revised Round 8): **22 rows with 0 phantoms + 5 missing**. PR 4 commit 9 adds ONLY **1 residual row** (`production_session_add`); v2.0 phase PR adds 3 more (`bdd_obligation_sync`, `bdd_no_direct_call_impl`, `test_marker_coverage`) AND DELETES `test_architecture_no_silent_except.py` (Round 8 drift-verified). The full ~81-row table audit DEFERS to a post-v2.0-rebase commit.
3. Guard count math (D18 revised Round 8): 27 baseline + 1 (PR 2) + 4 (PR 4) + 1 (PR 5) + 8 (PR 1/3/6 governance) + 27 (v2.0 architecture/) + 4 (v2.0 top-level) + 9 (v2.0 baseline JSONs) = **~81 final post-v2.0-rebase**.
...
6. Hooks MOVED to `pre-push` (10 — D27 revised Round 8): check-docs-links, check-route-conflicts, type-ignore-no-regression, adcp-contract-tests, mcp-contract-validation, mcp-schema-alignment, check-tenant-context-order, ast-grep-bdd-guards, check-migration-completeness, **mypy** (the 10th — added per D3; CI's `CI / Type Check` job is authoritative).
