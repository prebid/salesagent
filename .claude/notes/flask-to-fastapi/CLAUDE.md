...
1. **`script_root` template breakage.** Starlette's `include_router(prefix="/admin")` does NOT populate `scope["root_path"]` the way Flask's blueprint mounting populated `request.script_root`. ~147 template references break silently. **Fix:** the `render()` wrapper in the foundation module injects `admin_prefix="/admin"` and `static_prefix="/static"` as Jinja globals; a codemod rewrites templates. See `flask-to-fastapi-deep-audit.md` §1 (blocker 1).
...
- **Templates use `admin_prefix` / `static_prefix`, not `script_root`.** A migration-only Jinja convention introduced by the `render()` wrapper. The rest of the codebase has no equivalent because the rest of the codebase does not render Jinja.
