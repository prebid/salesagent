...
| `src/core/database/queries.py` | 282 LOC; 7 sync functions taking `session` as parameter, each uses `session.scalars(...)` / `session.execute(...)` | Add `async def` variants; all callers use `await`. | +50 | LOW |
