...
| **NEW: lifespan startup threadpool tune** | `anyio.to_thread.current_default_thread_limiter().total_tokens = int(os.environ.get("ADCP_THREADPOOL_TOKENS", "80"))` — canonical env name per `foundation-modules.md §11.14.F` (the older `ADCP_THREADPOOL_SIZE` draft name is deprecated). | +5 | LOW |
