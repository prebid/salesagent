...
**Status:** ✅ COMPLETE at `a2d3b350` (L0-00..L0-32, 33 items landed, 2026-04-19). All exit-gate work items shipped; next entry point is L1a.
...
2. [✅ DONE — foundation modules landed across L0-04..L0-15; see per-sub-item SHAs below] Create foundation modules (ALL sync `def`) — decomposed into 11 sub-items below [§4 Wave 0, §2 Blockers 1-2, §3.5.1 SB-3].
    ...
    **2.5** [✅ DONE — L0-07 `50ad5a1b`] `src/admin/external_domain.py` (~90 LOC) — `ApproximatedExternalDomainMiddleware`. Reads `Apx-Incoming-Host`; if present and differs from configured `SALES_AGENT_DOMAIN`, issues a **307** redirect (preserves POST body) to the tenant subdomain. Path-gated — never fires on `/mcp`, `/a2a`, `/_internal`. See `foundation-modules.md` §11.8.
    ...
    **2.6** [✅ DONE — L0-08 `eeb4f094`] `src/admin/proxy_headers.py` (~40 LOC) — `FlyHeadersMiddleware`. Rewrites `Fly-Forwarded-Proto` → `X-Forwarded-Proto` when `Fly-Forwarded-Proto` present and `X-Forwarded-Proto` absent. Complements uvicorn's `--proxy-headers` flag (which handles standard `X-Forwarded-*` but not `Fly-*`). See `foundation-modules.md` §11.9.
    ...
    **2.9** [✅ DONE — L0-11 `ee99874d`] `src/admin/oauth.py` (~60 LOC) — Authlib `starlette_client.OAuth()` registration for Google and per-tenant OIDC providers. Callback URI constants pinned: `/admin/auth/google/callback`, `/admin/auth/oidc/callback`, `/admin/auth/gam/callback` (byte-immutable with Google Cloud Console). See `foundation-modules.md` §11.6.
    ...
    **Exit gate for Work Item 2 (all 11 sub-items):** ✅ SATISFIED at `a2d3b350`
    - [x] All 11 modules merged (may land as 1-3 PRs; within-PR ordering is irrelevant, ordering of PRs against L1 is what matters)
    - [x] `test_foundation_modules_import.py` green (imports all 11 modules without ImportError) — L0-16 `f2d5e388`
    - [x] Golden-fingerprint capture via `capture-fixtures` skill runs successfully against the pre-L1 Flask admin — this proves the fingerprint infrastructure (Work Item 10) is operational — L0-21 `c736f6c5` + baselines `a2d3b350`
    ...
    - [x] Each module's `Done =` criteria above verified
3. [✅ DONE — L0-17 `fe27e9dd`] Create `form_error_response()` shared helper for DRY form-validation re-rendering across 25 routers [§3.5.6 EP-3].
4. [✅ DONE — L0-18 `32f09057`] Create feature flag routing toggle `ADCP_USE_FASTAPI_ADMIN` (~50 LOC) [§3.5.6 EP-1].
5. [✅ DONE — L0-18 `32f09057`] Create `X-Served-By` header middleware (~20 LOC) [§3.5.6 EP-2].
6. [✅ DONE — L0-20 `d9de5b43`] Write `scripts/generate_route_name_map.py` (~50 LOC) — introspects Flask `url_map` [§2-B1].
7. [✅ DONE — L0-20 `d9de5b43` (written, NOT executed — execution deferred to L1a per §7.5 RATIFIED)] Write `scripts/codemod_templates_greenfield.py` (~200 LOC) — Pass 0 (csrf, g.*, flash), Pass 1a (static), Pass 1b (hardcoded paths), Pass 2 (Flask-dotted names) [§2-B1].
8. [✅ DONE — L0-20 `d9de5b43` + guards `b1ba541f` + idempotency obligation `c558e983`] **Write** `scripts/codemod_templates_greenfield.py` is complete but do **NOT run it** in L0 — all 4 passes break Flask's `url_for` while Flask still serves traffic. Execution moves to L1a. Manual audit of `add_product_gam.html`, `base.html`, `tenant_dashboard.html` deferred to L1a [§2-B1].
9. [✅ DONE — documented in worked-examples.md] Document `request.form.getlist()` → `List[str] = Form()` migration pattern in worked examples [§3.5.7 CP-2].
10. [✅ DONE — L0-21 `c736f6c5` + baselines captured at `a2d3b350`] Write golden-fixture capture infrastructure: `tests/migration/fingerprint.py`, `tests/migration/conftest_fingerprint.py`, `tests/migration/test_response_fingerprints.py`, `tests/migration/fixtures/fingerprints/*.json` [§3.5.5 TI-1]. Uses Starlette `TestClient` (sync).
11. [✅ DONE — L0-22 `1fa85750` + harness-overrides guard `fddda5e6`] Add harness extension: `IntegrationEnv.get_admin_client()` returning Starlette `TestClient` with `dependency_overrides` snapshot/restore [§4 Wave 0 / L0].
12. [✅ DONE — L0-02 `745c920e`] Write `tests/integration/test_schemas_discovery_external_contract.py` [§3 audit action #4].
13. [✅ DONE — L0-26 `904eb868` (`pydantic-settings` bump + OAuth URIs runbook + customer-comms plan)] Complete §1.1 prerequisites: `SESSION_SECRET` in `.env.example` and secret stores, OAuth URI docs, external consumer contract confirmation [§1.1].

**L0 work item — Pydantic v2 guard (native-ness, empty allowlist):** [✅ DONE — L0-23/L0-24/L0-25 tranche]
...
**Exit gate:** ✅ SATISFIED at `a2d3b350` (L0-00..L0-32, commit range `d2841632..a2d3b350`)
```bash
...
### L6 Phase-F greenfield-completion items (Agent ζ 2026-04-17)

The core L6 scope (items 1-5 above) ships the post-async cleanups that were blocked by L5. L6-NEW-* items below are greenfield-nativeness polish pulled forward from post-v2.0 work — they close the remaining gap between L7 end-state and "would be the same if we had implemented today without Flask in the first place." Ship them in L6 so L7 bakes on a fully greenfield codebase.

**L6-NEW-1 — MessagesDep verification (no-op; landed at L0 per D8 #4).** Verify `src/admin/deps/messages.py::MessagesDep` exists and is the canonical flash path since L1a. Acceptance: `python -c "from src.admin.deps.messages import MessagesDep, Messages, FlashMessage"` succeeds; no `src/admin/flash.py`. Effort: 0h.

**L6-NEW-2 — Router-level `dependencies=[Depends(require_tenant_access)]`.** 14 tenant-scoped routers currently declare `Depends(require_tenant_access)` per handler. Refactor each to `APIRouter(prefix="/tenant/{tenant_id}/<feature>", dependencies=[Depends(require_tenant_access)], redirect_slashes=True, include_in_schema=False, tags=[...])` so the dep applies at router construction. Acceptance: zero per-handler `Depends(require_tenant_access)` in the 14 routers; `AdminTenantDep` continues to resolve `tenant_id` from `request.state.principal.tenant_id` unchanged. Structural guard: `tests/unit/test_architecture_tenant_scoped_router_deps.py` (empty allowlist). Effort: 3h.

**L6-NEW-3 — Explicit A2A agent-card routes (replace `_replace_routes()` mutation).** Current `src/app.py:192-215` mutates `app.router.routes` at module import to install dynamic agent-card handlers. Replace with: (a) call `a2a_app.routes(...)` non-mutating variant; (b) filter out the three agent-card paths; (c) register our own `Route` objects explicitly in `src/routes/agent_card.py`; (d) delete `_replace_routes()` function. Acceptance: `src/app.py` has no `app.router.routes = ...` assignment and no `.insert/.append/.extend` on the route list; agent-card paths return tenant-aware JSON byte-identical to pre-refactor. Structural guard: `tests/unit/test_architecture_no_route_list_mutation.py` (empty allowlist). Effort: 4h.

**L6-NEW-4 — ORJSONResponse default.** Add `orjson>=3.10` to `pyproject.toml`; construct `FastAPI(default_response_class=ORJSONResponse)` in `src/app.py`. All JSON responses become orjson-serialized (2-5× throughput on MCP/A2A/webhook payloads). OpenAPI byte-stability unchanged (orjson emits ordered keys). Acceptance: `orjson` in deps; `default_response_class` set; perf re-measured at L6 exit shows ≥ 5% throughput improvement on REST `/api/v1/*` + MCP tool-list endpoints. Structural guard: `tests/unit/test_architecture_default_response_class_is_orjson.py` (empty allowlist). Effort: 1h.
...
### L7 Phase-F greenfield-completion items (Agent ζ 2026-04-17)

Final polish that closes the last Flask-era artifacts. These run in parallel with the allowlist ratcheting + tag prep (items 1-18 above) during the L7 1-week bake.

**L7-NEW-1 — Verify `render()` deletion at L7 exit.** `src/admin/templating.py::render` wrapper was deleted at L4. L7 verifies it didn't regress. Acceptance: `rg 'from src.admin.templating import render' src/` returns 0; `rg 'render\(request,' src/` returns 0 (all calls should have become `templates.TemplateResponse(request, ...)` at L4). Structural guard: `tests/unit/test_architecture_no_render_wrapper.py` (empty allowlist; asserts `src/admin/templating.py::render` doesn't exist AND no module imports the name). Effort: 1h verification.

**L7-NEW-2 — NextURL AfterValidator annotation.** Replace 4 call-site reads of `safe_next_url(request.query_params.get("next"))` with Pydantic's `Annotated[str | None, AfterValidator(safe_next_url), Query()]` in handler signatures. Safer (validation runs before handler body) and more readable. Sites: `src/admin/routers/auth.py` (2), `src/admin/routers/public.py` (2). Acceptance: `rg 'safe_next_url\(request\.query_params\.get' src/` returns 0; 4 handler signatures gain the `SafeNextURL = Annotated[str | None, AfterValidator(safe_next_url), Query()]` alias. Effort: 2h.

**L7-NEW-3 — Delete `tests/migration/fingerprints/` + most of `tests/migration/`.** Fingerprint infrastructure was essential L1 parity tooling. Post-L5 the fingerprints describe a surface that no longer exists. Delete `fixtures/fingerprints/`, `fingerprint.py`, `conftest_fingerprint.py`, `test_response_fingerprints.py`, `dual_stack_client.py`, `baseline-sync.json`, `baseline-l5d2.json`, `baseline-l5e.json`. Move `test_async_performance_parity.py` → `tests/integration/`. Structural guard: `tests/unit/test_architecture_no_migration_fingerprints.py` (empty allowlist). Effort: 2h.

**L7-NEW-4 — File v2.1 ticket for LegacyAdminRedirectMiddleware sunset.** The 60-LOC 308-redirect middleware is a v2.0 migration compat layer for operator bookmarks. After 1 post-v2.0 release cycle, most bookmarks have migrated; the middleware can be removed. File GitHub issue with acceptance criteria + deprecation-notice plan; add `# SUNSET: <issue-url>` docstring to `src/admin/middleware/legacy_admin_redirect.py`. Acceptance: issue filed (URL in L7 exit checklist); module docstring updated. Effort: 1h.

**L7-NEW-5 — Route-naming codemod `admin_<blueprint>_<endpoint>` → `<feature>.<action>`.** The `admin_` prefix is redundant (entire surface is admin); dotted `<feature>.<action>` is greenfield-native. Codemod both Python (`@router.X(..., name=...)`) and templates (`{{ url_for('...') }}`). Acceptance: `rg 'name="admin_' src/admin/routers/` returns 0; `rg "url_for\\('admin_" src/admin/templates/` returns 0; existing `test_templates_url_for_resolves.py` continues to pass. Structural guard: `tests/unit/test_architecture_route_naming_convention.py` (empty allowlist; regex `^[a-z][a-z0-9_]*\.[a-z][a-z0-9_]*$`). Effort: 8h (codemod + verify + guard). **Consider moving to L6 if L7 bake is tight.**

**L7-NEW-6 — Verify `log_auth_cookies` / SESSION_DEBUG deletion.** Flask-era `@app.after_request` hook at `src/admin/app.py:272-295` dumped cookie state for debugging. Deleted with Flask removal at L2. L7 verifies no post-L2 code replicates the pattern (structlog `request_id` covers the telemetry need). Acceptance: `rg 'SESSION_DEBUG' src/` returns 0; `rg 'log_auth_cookies' src/` returns 0; `rg 'def log_\\w*_cookies' src/` returns 0. Structural guard: `tests/unit/test_architecture_no_cookie_debug_hooks.py` (empty allowlist). Effort: 0.5h verification.

**L7-NEW-7 — Per-status error template collapse.** Flask-era per-status templates (`403.html`, `404.html`, `500.html`) → single `error.html` + optional `error_partial.html` (HTMX). Exception handlers render with `status_code` in context; CSS conditions on the status. Acceptance: `src/admin/templates/403.html`, `404.html`, `500.html` deleted; `error.html` + `error_partial.html` exist; AdCPError handler + HTTPException handler + RequestValidationError handler all render `error.html` with `status_code` context. Structural guard: `tests/unit/test_architecture_single_error_template.py` (empty allowlist; asserts per-status files do not exist AND `error.html` does). Effort: 4h.

**L7 Phase-F total effort:** ~18.5h additional work (~2.3 engineer-days). L7's 3-5 day budget absorbs this; if L7-NEW-5 (route-naming codemod, 8h) is tight, move it to L6 with the related router-level `dependencies=[...]` refactor (L6-NEW-2).
