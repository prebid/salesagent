# Flask → FastAPI v2.0.0 — Execution Plan (Sync)
...
**Last updated:** 2026-04-12 (sync rewrite — async pivot reversed)

> **Async SQLAlchemy deferred to v2.1.** The async-audit reports in `async-audit/` contain
> comprehensive research for the v2.1 async migration. The v2.0 migration focuses solely
> on Flask removal with sync admin handlers. No `asyncpg`, no `async_sessionmaker`, no
> `expire_on_commit=False`, no lazy-load audit, no `SessionDep`, no `AsyncSession`. Admin
> handlers use sync `def` with `with get_db_session() as session:`. FastAPI runs sync
> handlers in its default threadpool — no `run_in_threadpool` wrappers needed for DB access.
> Testing uses Starlette `TestClient` (sync), not httpx `AsyncClient`. Factory-boy works
> as-is with no async shim.
...
> - `async-audit/` — **v2.1 reference only**; do not implement anything from these reports in v2.0
...
## Phase 3 — Cache migration + Flask removal + v2.0.0 release (~2,500 LOC, ~5-7 days)

**Goal:** Delete Flask entirely. Migrate flask-caching to SimpleAppCache. Tag `v2.0.0` and deploy. This is the v2.0.0 release phase.
...
15. Migrate 6 critical invariants from planning docs to code comments in `src/app.py` and `src/admin/app_factory.py` [§senior eng audit].
16. Bump `pyproject.toml` version to `2.0.0`, finalize `CHANGELOG.md` [§4 Wave 3].
17. Apply `v2.0.0` tag [§4 Wave 3].
18. Production deploy + 48h monitoring: error rates, latency p50/p99, Docker size, cookie size [§6].
19. Ratchet `.duplication-baseline` [§4 Wave 3].
20. Update auto-memory `flask_to_fastapi_migration_v2.md` to reflect completion.
...
**What NOT to do:** Do not start async conversion (that is v2.1). Do not remove `psycopg2-binary` (stays as the production driver). Do not remove `FLASK_SECRET_KEY` dual-read (v2.1). Do not drop nginx (v2.1). Do not design multi-worker scheduler (v2.2).

---

## Phase 4 (OPTIONAL) — Post-release cleanup

**Goal:** Prune planning artifacts, file v2.1 tickets (including async SQLAlchemy migration), archive the migration branch.

**Prerequisites:** Phase 3 complete. v2.0.0 deployed and stable >= 1 week in production.

**Knowledge to read:**
- `async-audit/` — all reports (for filing v2.1 tickets with accurate scope estimates)
- `implementation-checklist.md` §6-7 — post-migration verification + cleanup

**Work items (in order):**

1. File v2.1 tickets for deferred work:
   - Async SQLAlchemy migration (`create_async_engine`, `async_sessionmaker`, `AsyncSession`, async repositories, `asyncpg` driver) — the `async-audit/` reports contain comprehensive research, scope estimates, spike plans, and risk matrices for this work
   - Drop nginx (cannot happen until external-domain handling is battle-tested)
   - REST routes ratchet to `Annotated[...]` form
   - `Apx-Incoming-Host` IP allowlist
   - `require_tenant_access` to check `is_active`
   - `/_internal/` auth hardening
   - Hard-remove `FLASK_SECRET_KEY` dual-read
   - ContextManager singleton refactor (Decision 7 from async-audit)
   - SSE route deletion (Decision 8 from async-audit — orphan code)
   - `database_schema.py` deletion (confirmed orphan)
   - `product_pricing.py` deletion + DTO inlining (Decision 5)
   - Background sync service async refactor (Decision 9)
2. Prune planning docs: remove execution details that are now historical. Keep `CLAUDE.md` (notes) as a v2.1 reference pointer.
3. Archive `.claude/notes/flask-to-fastapi/` planning artifacts (or move to `docs/history/v2.0-migration/`).
4. Delete `feat/v2.0.0-flask-to-fastapi` branch after merge confirmation.
5. Remove migration breadcrumb from root `CLAUDE.md`.
6. Update auto-memory to mark migration complete and point to v2.1 tickets.

**Files to modify:** `CLAUDE.md` (root), various doc files.
**Files to delete/archive:** `.claude/notes/flask-to-fastapi/` contents (after promoting anything worth keeping).

**Exit gate:**
```bash
# All v2.1 tickets filed with scope estimates from async-audit/ reports
# Planning artifacts archived or deleted
# Branch deleted
# Root CLAUDE.md no longer references active migration
```

**What NOT to do:** Do not start v2.1 async work in this phase — just file the tickets. Do not delete the `async-audit/` reports (they are the v2.1 research base). Do not rush this phase — it is optional housekeeping, not a deadline.
