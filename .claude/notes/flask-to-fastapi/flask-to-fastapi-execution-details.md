...
27. **Decision 9 (background_sync sync-bridge) — full implementation criteria:**
    27.1. `src/services/background_sync_db.py` (NEW, ~200 LOC) exists with:
       ...
       - Module docstring documenting the sunset policy (post-v2.0 deletion target once `background_sync_service.py` is rewritten as a proper async service)
...
5. `psycopg2-binary` **RETAINED** in `pyproject.toml` runtime deps per Decisions 1 (Path B sync factory), 2 (pre-fork orchestrator health check), and 9 (sync-bridge). Removal deferred to **post-v2.0** when Path B adapters go async, sync-bridge sunsets, and `run_all_services.py` is replaced by a proper process supervisor. `types-psycopg2` also RETAINED. `libpq-dev` + `libpq5` RETAINED in Dockerfile.
