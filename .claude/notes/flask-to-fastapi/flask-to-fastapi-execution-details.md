...
6.5. **Spike 5.5 (Checkpoint-session viability — soft blocker, 0.5 day; D3 2026-04-16 supersedes pre-D3 "Two-engine coexistence" spike):** Validates Decision 9 resolution under D3 async rearchitect. Proves the async sessionmaker pool sustains high per-tick checkpoint session churn without `QueuePool limit` errors, cancellation leaks, or state loss. Writes 4 test cases at `tests/driver_compat/test_checkpoint_session_viability.py`: **(a) 4-hour sync with per-page short-lived sessions** — mock GAM pagination returning ~500 pages × 30s each; each page opens its own `async with get_db_session()`, writes a `sync_checkpoint` row, commits, closes; verify no `QueuePool limit` and final checkpoint row matches last page cursor; **(b) 3 concurrent multi-tenant syncs** — share the async pool without contention or deadlock; each tenant's checkpoint row is independent; **(c) Cancellation via `task.cancel()`** — active `asyncio.Task` receives `CancelledError` mid-page; cancel-handler writes final checkpoint BEFORE engine disposal (B8 mitigation); subsequent resume from checkpoint is clean; **(d) Resume from persisted checkpoint** — container restart mid-sync; new process reads `sync_checkpoint` row; continues from next cursor without reprocessing committed pages. **Pass:** all 4 green; no `QueuePool limit` under p95 load; no cancellation-checkpoint race. **Fail (SOFT):** revert to pre-D3 Option B sync-bridge (retain psycopg2-binary broadly; file v2.1 sunset ticket) — document in `spike-decision.md`. (Pre-D3 Spike 5.5 was "Two-engine coexistence" with tests at `tests/driver_compat/test_sync_bridge_coexistence.py` validating asyncpg + psycopg2 engine coexistence — superseded by D3 since no sync-bridge engine is created.) Also verifies the Wave 3 `from flask import current_app` fix at `src/services/background_sync_service.py:472` — spike owner confirms the replacement uses `src/admin/cache.py::SimpleAppCache` helper (see Decision 9 and Wave 3 acceptance criteria). Verifies grep shows 3 `flask-caching` consumer sites (inventory.py:874, inventory.py:1133, background_sync_service.py:472) — NOT zero — so the Wave 3 "delete flask-caching" plan is corrected to "replace with SimpleAppCache then delete".
