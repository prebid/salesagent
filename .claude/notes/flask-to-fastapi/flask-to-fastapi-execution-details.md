...
9. **Spike 8 (Go/no-go decision gate):** a written decision meeting note committed at `.claude/notes/flask-to-fastapi/spike-decision.md` summarizing each spike's pass/fail state, including `spike-results.log` excerpts for Spike 1, `baseline-sync.json` for Spike 3, Spike 4.5's LOC-count + error-path composition evidence, Spike 5.5's 4 test results, and the final go/no-go call. Go condition: **Spike 1 PASSES** AND no more than 2 soft spikes fail (out of Spikes 2-7 + 4.5 + 5.5 — total 8 soft spikes). Spike 4.5 and Spike 5.5 are SOFT blockers; fail actions fall back to Wave 4a sub-phase PR (Spike 4.5) or Option A asyncio task (Spike 5.5), not pivot-level reverts. No-go condition: Spike 1 FAILS OR more than 2 soft spikes fail.
...
5. `flask`, `flask-socketio`, `python-socketio`, `simple-websocket`, `waitress`, `a2wsgi`, `types-waitress` removed from `pyproject.toml`. **`flask-caching` is NOT deleted in Wave 3 — it is replaced first, then deleted (Decision 9 correction).** The Wave 3 flask-caching plan in migration.md §11.7 claimed "zero callers" but 3 consumer sites exist at `src/admin/blueprints/inventory.py:874`, `src/admin/blueprints/inventory.py:1133`, and `src/services/background_sync_service.py:472`. Wave 3 ships a replacement first: `src/admin/cache.py::SimpleAppCache` (~40 LOC `cachetools.TTLCache` wrapper on `app.state.inventory_cache`, with a `get_app_cache()` helper for background-thread access from `background_sync_service.py`). The 3 consumer sites migrate to the new cache. ONLY THEN is `flask-caching` removed from `pyproject.toml`. Grep `from flask import current_app` across `src/services/` returns zero hits after this step — this also closes the Wave 3 ImportError blocker at `background_sync_service.py:472`. `uv lock` succeeds.
5.1. `/Users/quantum/Documents/ComputedChaos/salesagent/src/admin/cache.py::SimpleAppCache` exists (~40 LOC), `cachetools.TTLCache`-backed, thread-safe via `threading.RLock`, install hook `install_app_cache(app)` called from the FastAPI lifespan startup, global-lookup helper `get_app_cache()` for background-thread access.
5.2. `src/admin/blueprints/inventory.py:874, 1133` consumer sites migrated from `getattr(current_app, "cache", None)` to `request.app.state.inventory_cache`.
5.3. `src/services/background_sync_service.py:472` consumer site migrated from `from flask import current_app` + `current_app.cache.delete(...)` to `from src.admin.cache import get_app_cache` + `get_app_cache().delete(...)`. The `from flask import current_app` line is DELETED in the same commit.
...
15. All services-layer files from Agent A §2.5 converted: `delivery_webhook_scheduler.py` + `media_buy_status_scheduler.py` tick bodies (`with get_db_session()` → `async with`, repository calls `await`-ed), plus `auth_config_service.py` (10 uses — largest), `background_sync_service.py` (9 uses), `order_approval_service.py` (7 uses), and the 13 smaller services. `src/services/background_sync_service.py` handling locked in per pre-Wave-0 decision #9 (session-per-tick vs session-per-job).
...
25. **Decision 1 (adapter Path B) — full implementation criteria:**
    ...
    25.4. `src/app.py::lifespan` adds `anyio.to_thread.current_default_thread_limiter().total_tokens = int(os.environ.get("ADCP_THREADPOOL_SIZE", "80"))` at startup — the default 40 workers is too low for burst adapter load per Decision 1 analysis. Env-override via `ADCP_THREADPOOL_SIZE`.
...
27. **Decision 9 (background_sync sync-bridge) — full implementation criteria:**
    27.1. `src/services/background_sync_db.py` (NEW, ~200 LOC) exists with:
       - Module-level lazy-initialized sync psycopg2 engine, `application_name='adcp-salesagent-sync-bridge'`, pool `pool_size=2, max_overflow=3, pool_pre_ping=True, pool_recycle=3600`, statement_timeout=**600s** (10-minute budget for long GAM syncs)
       - `@contextmanager def get_sync_db_session()` yielding sync `Session`
       - `dispose_sync_bridge()` function for lifespan shutdown
    ...
    27.4. `src/services/background_sync_service.py::start_inventory_sync_background` is wrapped in `await run_in_threadpool(...)` when called from async admin handlers (e.g., `src/admin/blueprints/inventory.py::start_inventory_sync`). The service internals stay sync; only the crossing into the sync-bridge thread goes through the threadpool.
    27.5. Structural guard `tests/unit/test_architecture_sync_bridge_scope.py` lands with ratcheting allowlist (frozenset). Initial allowlist: `src/services/background_sync_service.py` ONLY. 4 test methods: (a) allowlist-not-empty implies module exists, (b) importers outside allowlist rejected, (c) allowlisted files that stop importing must be removed from allowlist, (d) stale allowlist entries (deleted files) caught.
    27.6. `pyproject.toml` — `psycopg2-binary` + `types-psycopg2` RETAINED as runtime deps (partial reversal of Agent F F1.1.1). `asyncpg>=0.30.0,<0.32` ADDED for the async path. Both drivers coexist.
    27.7. `Dockerfile` — `libpq-dev` (builder stage) + `libpq5` (runtime stage) RETAINED (partial reversal of Agent F F1.2.1). Docker image savings adjust from ~80MB to ~75MB.
