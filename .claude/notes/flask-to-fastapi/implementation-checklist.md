...
- [ ] **Agent F pre-Wave-0 hard gate items completed (non-code surface inventory, corrected 2026-04-11):**
  ...
  - [ ] **`DatabaseConnection` KEPT** (partial reversal of F1.4.1) per Audit 06 Decision 2 OVERRULE — `scripts/deploy/run_all_services.py:84,135` calls `get_db_connection()` as pre-uvicorn health checks. New structural guard `test_architecture_no_runtime_psycopg2.py` allowlists ONLY `db_config.py` + `background_sync_db.py` as legitimate psycopg2 importers.
