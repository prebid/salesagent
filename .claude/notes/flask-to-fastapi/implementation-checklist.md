...
- [x] **SB-3 [HIGH, Wave 0]: `session.*` and `g.*` template variables silently render as empty.** `templates/base.html:145` uses `{% if g.test_mode %}` (test mode banner); lines 155-183 read `session.role`, `session.authenticated`, `session.email`. Starlette does NOT auto-inject `g` or `session` into Jinja context. **Fix:** `render()` wrapper must pass `test_mode`, `user_role`, `user_email`, `user_authenticated`, `username` as explicit context variables. Add `test_template_context_completeness.py` guard asserting every variable used in `base.html` is present in `render()` context dict. — L0-05 `0c9ff48c` (initial 11-key BaseCtxDep) + L0-24 `2cce10d2` (template context completeness guard) + C1 pre-merge fix (collapsed to 10 keys — dropped dead `messages` dict key that was shadowed by `{% with messages = get_flashed_messages() %}` in all 6 base-rendering templates; `get_flashed_messages` is now sole flash surface)
...
- [x] **PRE-1: `src/admin/blueprints/oidc.py:173` — `test_initiate()` refactor.** Inline `get_or_create_auth_config` query against the outer `db_session` (mirror the existing fix at `callback()` line 229). Prevents detached-instance error under bare sessionmaker on `config.oidc_client_secret` getter. → landed commit `81ab9ace` (2026-04-16)
- [x] **PRE-2: `src/admin/blueprints/operations.py:304` — `approve_media_buy()` refactor.** Extract needed data to dict, close outer session before `execute_approved_media_buy`, open a fresh session for post-adapter status update (mirror `creatives.py:607-639` pattern). Prevents lost-update on `media_buy.status` under bare sessionmaker. → landed commit `81ab9ace` (2026-04-16; 329 → 277 LOC, three-phase extract-dict + close-outer)
- [x] **PRE-3: `src/admin/blueprints/workflows.py:158` — `approve_workflow_step()` refactor.** Same extract-dict + close-outer pattern as PRE-2. → landed commit `81ab9ace` (2026-04-16; regression test asserts adapter-set `status="active"` survives handler)
...
- [ ] **L4 native-ness: pydantic-settings centralization.** Extended existing `src/core/config.py` (NOT new `src/core/settings.py` — two settings modules is a Flask-era regression). All 107 `os.environ.get(...)` sites (verified 2026-04-17 via `rg 'os\.environ\.get' src -c`; previous estimate "89" was stale) consolidated into typed `BaseSettings` subclasses with `SettingsConfigDict(env_nested_delimiter="__")`. Credentials use `pydantic.SecretStr`. Guard `tests/unit/test_architecture_no_direct_env_access.py` seeded with 107 sites, ratchets to 0 by L7.
- [ ] **L4 native-ness: structlog adoption.** `structlog>=24.4.0` in `pyproject.toml`. 96 `print(` sites in `src/` (verified 2026-04-17 via `rg '^(\s+)?print\(' src -c`; previous estimate "121" was a broader regex that caught in-string/comment occurrences) replaced with `log = structlog.get_logger()` + `log.info(...)`. Pipeline: `merge_contextvars` + `TimeStamper(fmt="iso")` + `EventRenamer("message")` + `JSONRenderer()` (prod) / `ConsoleRenderer()` (dev). `RequestIDMiddleware` binds `request_id` via `structlog.contextvars.bind_contextvars(...)`. Guard `tests/unit/test_architecture_uses_structlog.py` blocks new `print(` in `src/**` (allowlist: `scripts/`, `alembic/versions/`, `src/core/cli/`).
