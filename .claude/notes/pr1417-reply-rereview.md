...
- **get_media_buys status is now date-refined** (`active→completed at flight end`) — a documented, pragmatic divergence, not a GA mandate. The GA lifecycle prose (media-buys/index.mdx:325, same MUST as beta.3:285) says to persist status and never recompute it from dates; it is silent on read-time refinement. Mainline #1545 chose to refine on read anyway as a best-current-view when the status scheduler lags the flight window, and this branch adopts that choice for parity — **routing `update_media_buy`'s `media_buy_status` through the same resolver** so the two tools agree.
