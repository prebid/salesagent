...
Read the body below as a dated record of the pre-#1721 branch, not as a description of this
tree. It lands here through the semantic merge of the signing epic onto PR #1721's request
boundary. The commit range in the header (`17ea737a4..a78a1a1b5`) is rebased away, so none of
§1's SHAs resolve, and §2's and §7's epic state is superseded twice over — first by
`.claude/notes/rfc9421-session-report-2026-07-29.md` (same eight tasks plus B3), then by the
merge. This note is kept because it is the detail record for those eight tasks, which is what
the 07-29 report cites it for.

**Statements the merge moved:**

- §1's module list names `operations.py` and `request_verifier_middleware.py`. Neither exists.
  Inbound verification is no longer an ASGI middleware: it runs inside the boundary's
  resolver (`src/core/signing/verifier.py`, reached from `src/core/tools/_boundary.py`).
  `src/core/signing/capture.py` records the bytes a signature covers without reading a
  credential, and `src/core/signing/vocabulary.py` holds the operation names `operations.py`
  held. The decision and its argument are in `docs/design/signing-vs-request-boundary.md`.
- §1's migration chain: `e7a2c40b91d5` survives. `04c04fef9503` and `b7c1d9f4a2e3` do not —
  the replay table and `principals.agent_url` arrive here as
  `c41f8a6b2d90_add_inbound_request_signature_verification.py`.
- §3's body-rewrite finding names `RestCompatMiddleware`, which #1721 deleted along with
  `src/routes/rest_compat_middleware.py`. The hazard it describes outlives the class, and is
  why `capture.py` replays every byte on every exit rather than consuming the receive channel.
- §4's `salesagent-i12h` is about `_PATH_TO_TOOL`. There is no per-transport path map here;
  `src/core/tools/registry.py` is the one route registry.
- §7 says B1 is inert because `request_signing` sits in `_UNBACKED_BLOCKS`. It does not:
  `src/core/schemas/capability_declarations.py` holds `content_standards`,
  `wholesale_feed_webhooks` and `offline_delivery_protocols` there, and `webhook_signing`
  moved to `_DERIVED_BLOCKS`.
- §6's suite counts and ratchet readings are measurements of the rebased-away branch. They
  grade nothing in this tree; the merge's own baselines are the authority here.

**Findings that still stand against the merged tree:** the spec corrections and the eight SDK
divergences in §3, everything in §5, and both live bugs in §4 — `ck_property_type` still
admits only seven values (`src/core/database/models.py:2439`), so `salesagent-t03g` is open,
and the `.example.com` publisher-domain fallbacks survive at `models.py:448`, `:458`, `:468`
for `salesagent-8t5p`.
