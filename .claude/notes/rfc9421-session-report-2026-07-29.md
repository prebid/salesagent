...
**Where B1 landed.** The verifier shipped here as an ASGI middleware. Re-engineering the layer onto
PR #1721's request boundary removed that module: `verify_inbound_signature` in
`src/core/signing/verifier.py` is now called from `_resolve_identity`
(`src/core/resolved_identity.py`), the one place every transport resolves identity. The findings
below are unchanged by the move; where a function was renamed, both names are given.
...
- **Gate placement.** The design said "before `verify_request_signature`" without saying where.
  ...
  (negatives 001/027/028). It sits inside the signed-request handler, after the bucket decision —
  `_handle_signed` then, `_verify_signed` in `src/core/signing/verifier.py` now.
...
and `request_signature_key_origin_*`. Those eight constants exist in `adcp.signing` and
`src/core/signing/verifier.py` wires the resolvers that raise them (`_BrandJsonJwksResolver`,
`_jwks_resolver`, `_map_brand_json_resolver_error`), but **no conformance vector exercises them**, so B3
...
(key provisioning) and `salesagent-98t2` (the unsigned sender). **Corrected for the merged tree:** D1 has since landed. `request_signing` is no longer in
`_UNBACKED_BLOCKS` — that set now holds only `content_standards`, `wholesale_feed_webhooks` and
`offline_delivery_protocols`, with `webhook_signing` in `_DERIVED_BLOCKS`
(`src/core/schemas/capability_declarations.py:163`). Inbound verification is gated instead by
`SigningSettings.verifier_enabled` (`src/core/config.py:383`), read at
`src/core/signing/verifier.py:225`.
...
B1's surface allowlist, B2's operation map, `posture.bucket_names`, and the brand-discovery codes in
