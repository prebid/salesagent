...
lifecycle: migration
description: >
  Show migration progress dashboard: Flask imports remaining, async vs sync
  handlers, structural guards status, current phase, and remaining work.
---
...
# Async vs sync handlers in routers
rg -c "async def " src/admin/routers/ --type py 2>/dev/null || echo "0"
...
# Async engine?
grep -q "create_async_engine" src/core/database/database_session.py && echo "DB: async" || echo "DB: sync"
```
...
- AsyncSession in database_session.py → Phase 4+
- v2.0.0 in pyproject.toml → Phase 5 complete
...
DB engine:         sync (async in Phase 4)
