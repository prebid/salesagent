...
description: >
  ...
  Generates async handlers with SessionDep, repository Deps, named routes,
...
3. `.claude/notes/flask-to-fastapi/async-pivot-checkpoint.md` lines 160-243 — target handler+repo pattern
...
| Async handlers | `async def` with `SessionDep`, repository Deps | `test_architecture_admin_routes_async.py` |
...
async def get_{name}_repo(session: SessionDep) -> {Name}Repository:
...
async def list_{name}(
    ...
    items = await repo.list_all(tenant_id, status=status)
...
5. Run `make quality` — it catches missing route names, Flask imports, sync handlers
...
8. Dependency overrides for yield-deps must be async generators, NOT lambdas: `async def _override(): yield session` — not `lambda: session`
...
- `/async-convert` — convert sync repositories to async
