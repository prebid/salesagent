...
description: >
  ...
  parity, raw dicts to typed fields, dead code, test DRY violations,
  redundant DB queries, error envelope compliance, and untyped exception
  usage. Saves review rounds by catching these before submission.
args: <pr-number>
---
...
from analysis of 102 comments across 14 merged PRs, plus the error-emission
architecture conventions established in PR #1306.
...
### K. Error tests asserting on reconstructed exceptions

> Established by the error-emission architecture (PR #1306). Wire envelope
> is the buyer-facing contract; reconstructed exceptions are lossy.

In changed test files that verify error paths:

1. **`isinstance()` on reconstructed error**: `isinstance(error, AdCPValidationError)`
   verifies the harness reconstruction layer, not the actual wire shape.
   `AdCPAuthenticationError` and `AdCPAuthorizationError` both map to
   `AUTH_REQUIRED` on the wire — reconstruction always picks the first.
   ```python
   # BAD — tests reconstruction, not wire
   assert isinstance(result.error, AdCPValidationError)
   assert result.error.error_code == "VALIDATION_ERROR"

   # GOOD — tests actual wire contract
...
2. **`.error_code` on reconstructed exception**: Same issue —
   `error.error_code` reads from the reconstructed object, not the wire.
   Use `result.wire_error_envelope` as the primary authority.

3. **Missing recovery hint assertion**: Error tests should verify the
   `recovery` field (`transient`, `correctable`, `terminal`) — it drives
   buyer-agent retry semantics.
...
### L. `_impl` functions building wire shape or raising untyped exceptions

> Established by the error-emission architecture (PR #1306). `_impl` raises
> typed subclasses; boundary translators build the wire envelope.

In changed `_impl` functions (any function ending in `_impl`):

1. **Inline error dict construction**: `_impl` functions should never build
   `{"errors": [...]}` or `{"adcp_error": {...}}` dicts — that's the
   boundary translator's job via `build_two_layer_error_envelope()`.
   ```python
   # BAD — _impl building wire shape
   return {"errors": [{"code": "VALIDATION_ERROR", "message": msg}]}

   # GOOD — _impl raises, boundary translates
   raise AdCPValidationError(msg)
   ```

2. **Raw `ValueError` where a typed subclass exists**: If a typed
   `AdCPError` subclass covers the error condition, use it. `ValueError`
   gets normalized to `AdCPValidationError` at the boundary, but the
   typed subclass carries correct `status_code`, `recovery`, and
   `error_code` without relying on the normalization fallback.
   ```python
   # BAD — relies on boundary normalization
   raise ValueError("budget must be positive")

   # GOOD — carries correct metadata
   raise AdCPBudgetTooLowError("budget must be positive")
   ```

3. **`Error(code=...)` inline construction**: Use the semantic subclass
   instead of constructing `AdCPError(message, error_code="...")` with
   an inline code string. The structural guard tests cap these sites and
   enforce a shrink-only ratchet.
