...
5. Middleware order: **Approximated runs BEFORE CSRF** (not after — counterintuitive but correct). Canonical stack grows by layer via the `MIDDLEWARE_STACK_VERSION` assertion (foundation-modules.md §11.36): L1a=7 → L1c=8 (adds `LegacyAdminRedirectMiddleware` INSIDE UnifiedAuth per D1 2026-04-16) → L2=10 (adds `TrustedHost` + `SecurityHeaders`) → L4+=11 (adds `RequestID` outermost). L2 shape: `Fly → ExternalDomain → TrustedHost → SecurityHeaders → UnifiedAuth → LegacyAdminRedirect → Session → CSRF → RestCompat → CORS`. `SecurityHeadersMiddleware` (§11.28) lands in the L2 PR, positioned INSIDE `TrustedHost` and OUTSIDE `UnifiedAuth`.
