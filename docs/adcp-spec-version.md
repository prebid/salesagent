...
6. Update the version prose in `README.md` (status note + "AdCP Compatibility")
   ...
   step with the pin.
7. Nothing to re-vendor for schema SHAPE: every shape consumer resolves through
   `tests/helpers/pinned_schema.py`, which reads the installed SDK's own tree, so
   it follows step 2's pin automatically (`tests/unit/test_pinned_schema_single_source.py`
   asserts the derivation is intact). Do NOT bump
   `tests/fixtures/adcp_schemas_pinned/_refresh.py`'s `PINNED_SHA` as part of a spec
   bump — that fixture's pin is deliberately independent, held for the error-code
   `suggestion`-text divergence only (see "Pinned schema sources" below and
   [#1883](https://github.com/prebid/salesagent/issues/1883)).
...
- `tests/unit/test_adcp_spec_version.py` — CI guard; also grades that the prose files below advertise the current pin and name no stale versions
