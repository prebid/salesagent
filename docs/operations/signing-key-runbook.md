...
The KEK is named by `SigningSettings.key_passphrase_env` (`ADCP_SIGNING_KEY_PASSPHRASE_ENV`),
which holds the **name of the environment variable** carrying the passphrase — the
passphrase itself is never a configuration value. It is read through
`SigningSettings.secret_from_env`, which is why the name is the setting and the secret is
not.

With no KEK configured, a `db:` mint **refuses** with `AdCPConfigurationError`. It does not
fall back to storing plaintext. The refusal's wire sentence comes from the error-code
table and names no knob; the knob to set rides on the error's typed details
(`ConfigurationDetails.tracked_by`), which is what the ops script and the admin flash
surface. Either transport exits with that, not a traceback, because the operator's next
action is a setting, not a bug report.
...
documents. `SigningSettings.grace_seconds` defaults to **600 s**, which is 2× the
...
(`/.well-known/governance-revocations.json`, served by `src/routes/well_known.py`) — a JWS general-JSON
...
No posture is read and no bucket is graded. Inbound verification is a call
`_resolve_identity` makes behind the tool boundary, and the flag is the first thing
`verify_inbound_signature` checks, so there is no verification middleware to disable and
nothing to restart. It is a kill switch and it is deliberately a flag, so that a rollback
is a flag flip and not a deploy.
