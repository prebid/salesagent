...
The AdCP-legacy HMAC scheme is the 3.x fallback and is **removed in AdCP 4.0**. Three
consequences worth planning around now:

- A `Bearer` registration is answered with `Authorization: Bearer <credential>` and no
  body signature of any kind — not the legacy HMAC digest, and not an RFC 9421
  `Signature` header either. Registering an `authentication` block selects the legacy
  arm, and a delivery is authenticated one way or not at all.
- **The legacy schemes are the two AdCP 3.1.1 names, and nothing else.** `Bearer` and
  `HMAC-SHA256`, spelled exactly as the pin spells them. A registration naming anything
  outside that pair — `Basic`, or a lowercase `hmac-sha256` — is **refused**: nothing is
  dialled, nothing is delivered, and a log names the stored scheme so your operator
  contact can find the row. A registration that cannot be authenticated correctly is not
  authenticated incorrectly instead.
- **An HMAC credential shorter than 32 characters is refused, not signed with a weak
  key.** The pinned schema puts `minLength: 32` on `credentials`, and a shorter one earns
  the same zero-attempt refusal. A short shared secret is a password, not a signature.

A refusal is not a delivery failure you can retry into: it is a verdict on the stored
registration, and it stands until the registration is corrected.
...
If you are looking for `docs/webhooks/migration-from-fragmented-senders.md`, that path is
named by a deprecation warning inside the `adcp` SDK's own `WebhookSender`. It points at
the SDK's documentation, it does not resolve in this repository, and this agent does not
emit it — our legacy arm calls the SDK's signing function directly and never constructs
that sender. If you see the warning, it came from your own SDK usage. This page is the
migration path for this agent.
