...
class TritonDigital(AdServerAdapter):
    ...
    def create_media_buy(
        ...
        # Honest capability declaration (AdCP honest-declaration contract): Triton can
        # in principle compile audio-property identifier types (station_id, podcast_guid,
        # etc.) to its native targeting, but that requires the property_list resolver
        # work tracked separately. Until then, reject property_list targeting up-front
        # rather than silently dropping it. Future: override
        # ``_check_property_list_supported`` to inspect the resolved identifier types
        # and accept the audio-capable ones.
