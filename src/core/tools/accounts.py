...
def _list_accounts_impl(
    ...
    if identity is None or identity.principal_id is None or identity.tenant_id is None:
        raise AdCPAuthRequiredError(
            "Authentication required for list_accounts", details={"suggestion": "Provide a valid authentication token"}
        )
...
async def _sync_accounts_impl(
    ...
    if identity is None or identity.principal_id is None or identity.tenant_id is None:
        raise AdCPAuthRequiredError(
            "Authentication required: sync_accounts requires a valid auth token.",
