...
class WebhookURLValidator:
    ...
    def _maybe_allow_localhost(is_valid: bool, error: str, *, allow_localhost: bool) -> tuple[bool, str]:
        """Override localhost-shaped SSRF failures when testing allows them.

        Scoped to the two genuinely loopback-specific rejection shapes: the
        exact ``localhost`` hostname (an entry in BLOCKED_HOSTNAMES) and a
        literal 127.0.0.0/8 address. Both needles are safe because neither is
        influenced by caller-supplied text: ``localhost`` is an exact-match
        ...
        interpolated into an unresolvable-hostname error message — admitting
