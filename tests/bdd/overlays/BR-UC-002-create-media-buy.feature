...
#
# AdCP 3.1.1 permits idempotency.supported=false, but the derivative adcp-req
# UC-002 scenario still grades supported=true replay behavior unconditionally.
# Keep the upstream scenario id live while reconciling its behavior with this
# seller's advertised false discriminant. scripts/compile_bdd.py applies this
# replacement in both wholesale and merge modes. The boundary overlay also
# replaces a fragile hand-counted over-max value with a token the bound Given
# step expands to exactly 256 valid-pattern characters.
