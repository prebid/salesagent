...
def given_request_registers_authenticated_webhook(ctx: dict) -> None:
    ...
    envelope (``params.configuration.task_push_notification_config``) on A2A. The env
    owns that placement (``BaseTestEnv.credential_registrations``,
    ``_a2a_message_send_body``); the scenario states only the intent.
    ...
    its credentials on a JSON-RPC method of its own with no skill invocation at all. So
    this Given asks for a claim about CREDENTIALS, not about one request, and opts the
    scenario into being graded at every location the transport offers
    (``BaseTestEnv.credential_registrations``).

    ON #1721's BOUNDARY BOTH A2A LOCATIONS ARE A RECORDED GAP, and this Given is what
    keeps the gap visible rather than absent. The escalation now reads the VALIDATED
    request inside ``invoke_tool`` (``src/core/signing/webhook_credentials.py``), so it
    sees neither the ``message/send`` protocol envelope — ``on_message_send`` routes on
    the DataPart's skill and never threads ``params.configuration`` into the request —
    nor ``tasks/pushNotificationConfig/set``, which the a2a-sdk answers through handlers
    that never call the boundary (``AdCPRequestHandler`` declines it outright). Both are
    named by ``src/core/signing/verifier.py`` § "What the boundary cannot see", whose own
    conclusion is that the fix is for those handlers to reach the same boundary rather
    than for a second verifier to exist. Grading both locations is how this file keeps
    saying so; dropping either would turn the gap into something that reads as coverage.
