...
def then_response_within_sla(ctx: dict) -> None:
    """Assert the response completed successfully, proving end-to-end latency is within SLA.

    The When step dispatched the request synchronously. A successful response
    with production-computed fields (media_buy_id, status, packages) proves the
    full pipeline — auth, validation, adapter call, DB persistence — completed
    within the test timeout (well under 15s). If any stage hung or exceeded
    SLA, the test framework's timeout would have killed the request.

    Asserts:
    - No error (pipeline completed without exception)
    - Result status is "success" (all stages passed)
    - Response is a CreateMediaBuySuccess (not error envelope)
    - media_buy_id is a non-empty string (production generated an identifier)
    - packages list is populated (production created package records)
    """
    from src.core.schemas._base import CreateMediaBuySuccess

    error = ctx.get("error")
    assert error is None, f"Expected a successful response (SLA requires completion, not failure), got error: {error}"

    result = ctx.get("response")
    assert result is not None, "No response recorded — the request did not complete"

    # Production-computed: the wrapper status proves the full pipeline finished
    assert result.status == "success", f"Expected status='success' (full pipeline completed), got '{result.status}'"

    # Production-computed: response type proves success envelope, not error
    assert isinstance(result.response, CreateMediaBuySuccess), (
        f"Expected CreateMediaBuySuccess response, got {type(result.response).__name__}"
    )

    # Production-computed: media_buy_id is generated by the impl, not echoed from request
    assert result.response.media_buy_id, "media_buy_id is empty — production should generate a unique identifier"

    # Production-computed: packages list proves adapter + persistence completed
    assert result.response.packages, (
        "packages list is empty — production should populate packages from adapter response"
    )
