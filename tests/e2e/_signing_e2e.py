...
#: :149-152) — the kid the row's OWN revoke() call reported, not a re-read.
...
def resolvable_signing_counterparty(live_server: dict, *, access_token: str) -> Iterator[Any]:
    ...
    with live_repo_session(live_server) as db:
        ...
        # BY TOKEN HASH. ``Principal`` stores ``sha256(token)`` and a displayable prefix,
        # never the plaintext, so ``filter_by(access_token=...)`` names a column that does
        # not exist. Same lookup the e2e conftest's own seed verification performs.
        principal = session.scalars(select(Principal).filter_by(token_hash=hash_token(access_token))).first()
        assert principal is not None, (
            f"no Principal in the live e2e database answers the access token {access_token!r}; the "
...
def _token_columns(token: str) -> dict[str, str]:
    """The two credential columns a ``Principal`` row carries for the plaintext *token*.

    The row stores ``sha256(token)`` plus a displayable prefix and NEVER the plaintext, so
    a caller-supplied literal has to be hashed before it can be seeded — and no row can
    hand its token back. ``access_token=`` used to be passed to ``PrincipalFactory``
    instead: that names a column that no longer exists and a field the factory does not
    declare, so it would seed a principal answering nothing.

    Both factory call sites below derive the pair HERE rather than each spelling it out —
    one definition of "what a known token looks like as columns", the same shape
    ``tests/integration/conftest_ci_seed.py`` uses for the CI seed.
    """
    from src.core.credentials import hash_token, token_prefix

    return {"token_hash": hash_token(token), "token_prefix": token_prefix(token)}
...
def _seed_buying_surface(env: Any, tenant: Any, *, access_token: str) -> None:
    ...
    out of the created row — which is now the only way round it can travel, since the row
    keeps only ``sha256(token)`` (:func:`_token_columns`). The seam's ``yield`` stays a
...
def provisioned_trust_root_tenant(
    ...
    *counterparty_principals* maps ``access_token -> agent_url`` and seeds one Principal
    ...
    (``_resolve_identity`` -> ``verify_inbound_signature`` -> ``_verify_signed`` ->
    ``_resolution_for``), so a module grading the INBOUND signed path needs a principal
    that carries one — which
    ...
    try:
        with live_db_env(live_server) as env:
            ...
            for access_token, agent_url in (counterparty_principals or {}).items():
                PrincipalFactory(tenant=tenant, agent_url=agent_url, **_token_columns(access_token))
...
def buyer_headers(token: str) -> dict[str, str]:
    ...
    """
    return {**credential_headers(token=token), "Content-Type": "application/json"}
...
def provision_signing_key_via_admin(base_url: str, *, tenant_id: str, alg: str = "ed25519") -> str:
    ...
    return _admin_post_expecting_flash(
        ...
        not_found_hint=(
            ...
            "KEK (ADCP_SIGNING_DEV_KEK) is missing. That flash carries CODE_TABLE's generic sentence for "
            "CONFIGURATION_ERROR plus the route's readout of ConfigurationDetails.tracked_by, which is the "
            "part naming the setting — read it rather than the leading sentence. "
        ),
    )


#: The manual delivery-webhook trigger's TWO outcome flashes
#: (``src/admin/blueprints/operations.py`` :565-568). The route flashes the first only when
