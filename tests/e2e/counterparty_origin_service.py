...
#: resolved ``AgentResolution`` — JWKS included — per ``agent_url`` for
#: ``SigningSettings.agent_resolution_ttl_seconds`` (an hour by default), in
#: ``src/core/signing/verifier.py``'s ``_resolution_for``, so a SECOND consumer of
