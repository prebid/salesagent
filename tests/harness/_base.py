...
class IntegrationEnv(BaseTestEnv):
    ...
    def setup_default_data(self, **tenant_kwargs: Any) -> tuple[Any, Any]:
        ...
        and self._principal_id from constructor. Extra ``tenant_kwargs``
        are forwarded to ``TenantFactory`` (e.g. ``human_review_required``)
        so env subclasses can seed tenant policy columns that the live
        e2e_rest server reads from the shared DB.
        """
