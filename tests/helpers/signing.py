...
* **The verifier kill switch** (:func:`verifier_disabled` and its monkeypatch-scoped
  wrapper :func:`inbound_verifier_disabled`), which exists because the verifier's
  DEFAULT is on. A seller that runs the verifier is a seller that "supports request
  signing", and security.mdx @ v3.1.1 :1465 then makes a 9421 signature MANDATORY on
  any request carrying webhook credentials — ``push_notification_config.authentication``
  or ``accounts[].notification_configs[].authentication`` — regardless of
  ``required_for`` membership. That is a real production obligation with its own tests.

  It is not, however, the obligation a test about SERIALIZATION or FORWARDING is
  grading, and such a test that happens to carry an ``authentication`` block would
  otherwise be refused before its subject ran. Those tests say so explicitly, by
  entering the kill switch, rather than quietly dropping the block their assertions
  are about.
...
def wire_origin(url: str) -> str:
    ...
    The PORT is the whole reason this is a function and not a slice: ``_target_uri``
    (``src/core/signing/capture.py``) rebuilds the authority from
...
#: translated the deprecated ``account_id`` → ``account`` (``src/core/request_compat.py``)
#: and ``RestCompatMiddleware`` (``src/routes/rest_compat_middleware.py``) then rewrote
#: the body downstream of the verifier, so the route graded "the verifier saw the WIRE
#: bytes, not the rewrite". BOTH modules are deleted on this architecture — nothing
#: rewrites a request body between the socket and the signature subject
#: (``src/core/signing/capture.py`` captures the bytes as read) — so the collision is
#: structurally absent rather than untested. What remains, and what callers use, is a
#: real body-carrying route.
...
def deployment_kek(monkeypatch: Any, name: str = "SALESAGENT_TEST_SIGNING_KEK") -> Iterator[None]:
    ...
    ``Settings`` object carrying ``key_passphrase_env`` is a process global, so the
    cached settings are dropped too and the next ``get_settings()`` rebuilds off the
    environment set above.

    ``raising`` is left at its default: this attribute is named as a STRING, so a
    rename in ``src.core.config`` is invisible to every checker, and a tolerant
    setattr here would leave the cache undropped and this helper silently doing
    nothing. That is exactly how the pre-merge spelling (``_config``) survived the
    rename to ``_settings``.
    """
...
def verifier_disabled() -> Iterator[None]:
    ...
    THE one expression of "flip the lever"; :func:`inbound_verifier_disabled` is this plus
    the environment, for the callers that have a ``monkeypatch``.
    """
...
def inbound_verifier_disabled(monkeypatch: Any) -> Iterator[None]:
    ...
    verify (``src/core/signing/webhook_credentials.py``, read at the boundary by
    ``src/core/tools/_boundary.py``), so the request never reaches the ingest gate
    those suites exist to grade.

    ``SigningSettings.verifier_enabled`` is the honest lever, and the ONLY one. The
    ...
    on incoming requests" and :func:`~src.core.signing.verifier.verify_inbound_signature`
    runs on every resolved identity (``src/core/resolved_identity.py``), so with the
    switch left on that declaration is false. Flipping the switch makes it true:
    ``verifier.py`` :225 returns ``None`` before any posture is read, and
    ``agent_level_posture`` / ``posture_from_declarations`` then report
    ``supported=False`` because the agent really does not verify.

    Both halves are needed and they are not redundant. The environment is what a
    REBUILD of the settings reads (any ``load_settings()`` during the test), and
    :func:`verifier_disabled` is what the ALREADY-RESOLVED object carries — a caller
    that resolved the settings before this helper ran sees only the latter.
    ...
    IN-PROCESS ONLY. This patches THIS process's settings, so it does nothing for a
...
# ---------------------------------------------------------------------------
# Tier-3 brand authorization (#1291 hksr) — NO PRODUCTION SEAM ON THIS ARCHITECTURE
# ---------------------------------------------------------------------------
#
# These four build the SDK's brand-authorization objects and reach nothing in ``src``.
# On the old architecture they were seeded into the verifier middleware's
# ``_BRAND_AUTHZ_RESOLVER_CACHE``, which made them a substitute for a production
# decision. That cache does not exist here: ``_resolution_for``
# (``src/core/signing/verifier.py``) delegates the whole counterparty walk to the SDK's
# ``resolve_agent`` and holds no authorization seam, so NOTHING IN ``src`` CONSULTS A
# RESOLVER A TEST CAN SUPPLY.
#
# They are kept because they are usable on their own terms — a test may drive the SDK's
# real ``BrandJsonAuthorizationResolver`` over a seeded transport and grade ITS binding
# logic. They must not be presented as configuring production: a caller that seeds one
# and then asserts on a request outcome is asserting on a resolver nothing read.
...
def counterparty_key(
    ...
    *,
    ...
    a seeded counterparty. That cache does not exist on this architecture:
    :func:`~src.core.signing.verifier._resolution_for` delegates the whole walk to the
    SDK's ``resolve_agent`` and holds no authorization seam of its own, so there is no
    production object to seed. Seeding a name that no longer exists is what this
...
def signing_config(**overrides: Any) -> Iterator[Any]:
    """Substitute the agent-level ``SigningSettings``, keeping every other field.

    :func:`~src.core.signing.verifier.verify_inbound_signature` reads
    ``get_settings().signing`` fresh on every request, so replacing what that call
    returns is what reaches ``_run_verifier``. Built by CONSTRUCTION rather than
    ``model_copy``, so an override naming a field that does not exist fails loudly
    instead of attaching a stray attribute — which is also what makes a red test for a
    not-yet-existing field fail for its own reason.

    Swapped at the module GLOBAL (``src.core.config._settings``) rather than by
    assigning ``settings.signing``. ``Settings`` is a FROZEN dataclass on this
    architecture: ``patch.object(settings, "signing", ...)`` raises
    ``FrozenInstanceError`` on the way in — and, worse, again on the way out while
    unwinding, so the original exception is masked. ``dataclasses.replace`` builds the
    whole settings object with one field substituted, which is the supported way to do
    this and keeps every other field identical by construction.

    The name check is EXPLICIT rather than left to Pydantic. ``SigningSettings`` is a
    ``BaseSettings`` declaring ``extra="ignore"`` (it has to: the process environment
    carries names no model owns), so construction SILENTLY DROPS an unknown keyword.
    Without this, ``signing_config(verifer_enabled=False)`` would yield the unmodified
    settings and every assertion under it would grade the default posture while
    reading as though it graded an overridden one.
    """
    from src.core.config import SigningSettings, get_settings

    unknown = set(overrides) - set(SigningSettings.model_fields)
    if unknown:
        raise ValueError(
            f"signing_config() got {sorted(unknown)}, which SigningSettings does not declare. "
            f"Known fields: {sorted(SigningSettings.model_fields)}. Refusing rather than "
            "ignoring them (extra='ignore'), which would silently yield the default settings."
        )

    settings = get_settings()
    replaced = SigningSettings(**{**settings.signing.model_dump(), **overrides})
    with patch("src.core.config._settings", dataclasses.replace(settings, signing=replaced)):
        yield replaced
...
def verifier_spy() -> Iterator[list[dict[str, Any]]]:
    ...
    ``_resolve_identity`` — and therefore ``verify_inbound_signature`` — off the loop);
    ``list.append`` is atomic under the GIL, so the recording list is safe as-is.

    Patched on ``src.core.signing.verifier``, which imports the SDK entry point by NAME
    at module scope (``verifier.py`` :100) and calls it through that module global in
    ``_run_verifier``. Patching ``adcp.signing.verifier`` instead would not be seen.
    """
...
def request_signature_codes() -> frozenset[str]:
    ...
    Read off the SAME source production reads — ``SignatureErrorCode``
    (``src/core/errors/signature_codes.py``), the enum whose 28 members ARE the
    ...
    ``_TAXONOMY`` as the SDK's 27 rows PLUS the ``request_target_uri_malformed`` row
    the SDK omits at ``adcp==6.6.0``, so the one-row correction below is no longer
    something this helper has to know about separately. The members are ``StrEnum``,
    so the returned set compares equal to the wire strings.
    ...
    through ``reject_malformed_target`` (``src/core/signing/canonical.py``, called
    from ``src/core/signing/verifier.py``), and that name carries no
    ``REQUEST_SIGNATURE_`` prefix, so the scan cannot see it. Measured, the enum's
    member set is EXACTLY the scan's 27 codes plus that one. The consequence was not
    ...
    passing for a webhook code would grade the wrong direction. The enum's MEMBERS
    are the request family; the webhook translations are the taxonomy's values and
    are not returned.
    """
    from src.core.errors.signature_codes import SignatureErrorCode

    return frozenset(SignatureErrorCode)
...
def scraped_verified_count(base_url: str, key_id: str, *, when: str = "now") -> float:
    ...
    verifier returns a ``VerifiedSigner``, so a non-zero total for a per-capability ``keyid``
...
def seed_principal(env: Any, *, agent_url: str | None = COUNTERPARTY_AGENT_URL) -> str:
    ...
    *env* is the live :class:`~tests.harness._base.BareIntegrationEnv`: it is not
    ...
    # what makes the token returned here the one that authenticates as this principal.
    return plaintext_token_for(SIGNING_PRINCIPAL_ID)
...
def request_headers(token: str | None, extra: dict[str, str] | None = None) -> dict[str, str]:
    ...
    headers = credential_headers(token=token, tenant=SIGNING_TENANT_ID)
    if extra:
...
def signed_headers(
    ...
    *origin* defaults to :data:`WIRE_ORIGIN`, the in-process ASGI client's origin, so
    ...
    origin **including the port**: ``_target_uri``
    (``src/core/signing/capture.py``) rebuilds the authority from
