...
async def test_lifespan_awaits_every_registered_shutdown_callback(isolated_global_app_state):
    ...
    Repointed by the #1802 seam migration. This used to prime a real connection
    pool on ``ProtocolWebhookService`` and assert the lifespan emptied it. That
    service now builds and discards a per-destination pinned transport, so it
    owns no pool and registers nothing — with it went the last producer in
    ``src/``, and with that, mutation (b) ("skip register_shutdown on
...
async def test_constructing_the_webhook_service_registers_no_shutdown_callback(isolated_global_app_state):
    ...
    self-register ``close`` to release it. The egress-seam migration (#1802)
    removed the pool, because ``build_ip_pinned_transport`` resolves its
    destination at construction and refuses to connect anywhere else — so there
    can be no client that outlives a single delivery, and therefore nothing left
...
async def test_lifespan_swallows_a_failing_shutdown_callback(isolated_global_app_state):
    ...
    Repointed by the #1802 seam migration for the same reason as the drain test above: the
