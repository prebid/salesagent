...
def test_v3_client_without_buying_mode_rejected_via_rest(env):
    ...
    # AdCPInvalidRequestError (400). Pin to == 400 so a server-side 500 (crash) regression
    # surfaces here instead of passing under a generic ">= 400" check.
    assert response.status_code == 400, f"Expected 400 INVALID_REQUEST, got {response.status_code}: {response.text}"
...
class TestBuyingModeRejectWireEnvelope:
    ...
    translates the AdCPInvalidRequestError into the spec two-layer envelope carrying the
    ...
    def test_wholesale_with_brief_rejected_with_suggestion(self, env, transport):
        """wholesale + brief rejects with INVALID_REQUEST + actionable suggestion at every wire transport."""
        result = env.call_via(
            transport,
            buying_mode="wholesale",
            brief="display ads",
