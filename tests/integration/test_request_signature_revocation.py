...
Each test below encodes the behavior the REFINED plan creates
...
under ``asyncio.to_thread`` (``src/core/tools/_boundary.py:333``), which is
...
The contract this suite grades
------------------------------

``src.core.config.SigningSettings`` (``get_settings().signing``, which is what
``verify_inbound_signature`` reads per request — the pre-merge ``SigningConfig``
is gone)
...
``src.core.signing.verifier``
  ``checker_for``                    imported into the verifier namespace and
                                     passed as ``VerifyOptions.revocation_checker``
                                     in ``_run_verifier`` (plan step 3). It is
                                     built in ``_verifier_dependencies``, so the
                                     wiring is exercised by every wire test here.
...
def _counterparty_at(origin: str, jwks: dict[str, Any]) -> Iterator[None]:
    ...
    NO TIER-3 RESOLVER IS SEEDED, and rebinding ``brand_json_url`` no longer has
    a second effect. The pre-merge middleware ran its own brand-authorization
    tier out of a ``_BRAND_AUTHZ_RESOLVER_CACHE`` keyed by this very field, so
    repointing it at an SSRF-blocked IP or an unresolvable ``.invalid`` host — a
    host chosen here for where the REVOCATION list is fetched from, not for
    anything about brand binding — needed an unconditional double under the
    rebound key. On #1721 :func:`~src.core.signing.verifier._resolution_for`
    delegates the whole walk to the SDK's ``resolve_agent`` and holds no
    authorization seam, so there is no production object to seed; a cached
    resolution inside its TTL is used as-is and the walk is never re-run. Seeding
    a name that no longer exists is what this refuses to do.
    """
    with counterparty_key(jwks):
        cached = AGENT_RESOLUTION_CACHE[COUNTERPARTY_AGENT_URL]
        rebound = cached.model_copy(update={"brand_json_url": f"{origin}/.well-known/brand.json"})
        with seeded_cache_entry(AGENT_RESOLUTION_CACHE, COUNTERPARTY_AGENT_URL, rebound):
            yield
...
class TestResolverReadsThroughTheResolutionCache:
    ...
    def _build_resolver(self) -> Any:
        ...
        """
        checker_for(AGENT_RESOLUTION_CACHE[COUNTERPARTY_AGENT_URL], SigningSettings())
    ...
    def test_two_counterparties_at_one_operator_origin_do_not_lock_each_other_out(self):
        ...
        with _counterparty_at(self._ORIGIN, first):
            resolver = self._build_resolver()
            neighbour = AGENT_RESOLUTION_CACHE[COUNTERPARTY_AGENT_URL].model_copy(
                update={"agent_url": _OTHER_AGENT_URL, "jwks": second}
            )
            AGENT_RESOLUTION_CACHE[_OTHER_AGENT_URL] = neighbour
            try:
                ...
                AGENT_RESOLUTION_CACHE.pop(_OTHER_AGENT_URL, None)
...
class TestCheckerForBuildsTheSpecCompliantChecker:
    ...
    def test_the_registry_reuses_one_checker_per_issuer_origin(self):
        ...
        with _counterparty_at(origin, jwks):
            resolution = AGENT_RESOLUTION_CACHE[COUNTERPARTY_AGENT_URL]
            checker_for(resolution, SigningSettings())
