...
- wrong-type revision ("not-an-int"): every transport emits
  ``INVALID_REQUEST`` at the shared request-schema boundary.
"""
...
class TestUpdateRevisionValidationWire:
    ...
    @pytest.mark.parametrize("transport", _WIRE_TRANSPORTS, ids=lambda t: t.value)
    def test_wrong_type_revision_emits_invalid_request_on_every_transport(self, env_with_media_buy, transport):
