"""Guard: the 5 salesagent-oyiv.1 Wave A/B migration targets must read the wire, not the
...
(``media_buy_deliveries`` / ``by_package`` / ``totals`` / ...) by its own docstring, and
salesagent-oyiv.6 tracks widening it as its own structural blind spot — this guard is not
that widening. It targets exactly the 5 functions salesagent-oyiv.1's revised Wave A + Wave B
plan migrates onto ``wire_dict(ctx)`` / ``wire_field(ctx, ...)``
...
elsewhere in the corpus (that is Check C's, and salesagent-oyiv.8/.9/.10's, job).
...
# Empty by design — every one of the 5 targets is a defect this ticket must eliminate, not
# an accepted/tracked exception. Today (TDD red, pre-migration) all 5 are still typed-payload
# reads, so this test currently reports 5 "new violations" and FAILS. salesagent-0bdd.7's
# migration must drive _find_violations() to the empty set so this test goes green — no
# allowlist entry should ever be added for these 5 (there is no legitimate reason to keep
# reading the typed payload here; unlike Check C, this guard has no permanent exceptions).
...
def test_no_typed_sandbox_or_array_oracle() -> None:
    """salesagent-oyiv.1 Wave A/B: these 5 functions must read wire_dict(ctx)/wire_field(ctx,
    ...), never a value off ctx['response']/_require_response(ctx)."""
    assert_violations_match_allowlist(
        _find_violations(),
        _ALLOWLIST,
        fix_hint=(
            "One of the 5 salesagent-oyiv.1 Wave A/B targets still reads a field value off "
...
def test_detector_flags_a_typed_payload_read_positive_control() -> None:
    ...
    assert _reads_typed_field(func, typed_vars), (
        ...
        "ctx['response'] local via getattr — this is exactly the pattern salesagent-oyiv.1 "
