...
def find_inline_admin_auth_violations(tree: ast.Module) -> list[int]:
    ...
    violations: list[int] = []
    # Attribution comes from the shared helper, not a second hand-rolled walk: four other
    # guards already pair walk_with_enclosing_function with iter_call_expressions this way.
    enclosing = {id(node): func_name for node, func_name in walk_with_enclosing_function(tree)}
