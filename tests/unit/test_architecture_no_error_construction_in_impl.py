...
PATTERN_A_PER_FILE_CAP: dict[str, int] = {
    # property_list-unsupported rejection: adapters return
    # CreateMediaBuyError(errors=[Error(code="UNSUPPORTED_FEATURE")]) following the
    # established return-based error convention (callers isinstance-check the
    # returned *Error result — adapters cannot raise). Same convention as the
    # sibling kevel/broadstreet/gam sites below.
    "src/adapters/base.py": 1,
    "src/adapters/broadstreet/adapter.py": 13,
    "src/adapters/google_ad_manager.py": 22,
    "src/adapters/kevel.py": 6,
