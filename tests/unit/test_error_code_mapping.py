...
2. Every mapped target is in WIRE_STANDARD_CODES (SDK STANDARD_ERROR_CODES +
   pinned-spec ``_SPEC_SUPPLEMENT_CODES``)
...
class TestErrorCodeMapping:
    ...
    def test_all_targets_are_standard(self):
        ...
        Targets may come from the SDK ``STANDARD_ERROR_CODES`` table or from
        ``_SPEC_SUPPLEMENT_CODES`` (pinned enum codes the SDK has not caught up
        to yet, e.g. REFERENCE_NOT_FOUND / CREATIVE_NOT_FOUND).
        """
