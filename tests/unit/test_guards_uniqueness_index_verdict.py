...
#: inventory makes every scan below vacuously green. The 19th declared tuple is
#: ``SigningKey.uq_signing_keys_tenant_kid`` on ``(tenant_id, kid)`` — a real
#: UNIQUE constraint (``e7a2c40b91d5_add_signing_keys_table``), not a PK subset:
#: ``signing_keys`` keys on a surrogate ``id``, so the natural key is separately
#: declared and separately enforced.
