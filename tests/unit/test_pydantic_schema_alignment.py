...
KNOWN_SCHEMA_LIBRARY_MISMATCHES: dict[str, set[str]] = {
    ...
    "/schemas/latest/media-buy/sync-creatives-request.json": {
        "account",  # Schema defines 'account' (AccountReference object); local model exposes both shapes for v3 transition. Tracked under #1247.
        "account_id",  # Schema defines 'account_id' (string); generator produces 'account_id' from updated schema cache. Tracked under #1247.
        "idempotency_key",  # Schema defines request deduplication key, library doesn't have it yet
    },
